...
bool computeNextCronOccurrenceForDate(
    const ESPDate &date, const Schedule &schedule, const DateTime &fromUtc, DateTime &outNextUtc
) {
...
bool computeNextSunOccurrenceForDate(
    const ESPDate &date,
...
bool moonPhaseCrossed(
	...
	return segmentIntersectsPeriodicWindow(
	    previousUnwrapped,
	    currentUnwrapped,
	    targetAngleDegrees,
	    toleranceDegrees
...
bool moonIlluminationCrossed(
    double previousValue, double currentValue, double targetPercent, double tolerancePercent
) {
	const double minWindow = targetPercent - tolerancePercent;
	const double maxWindow = targetPercent + tolerancePercent;
	const bool wasInside = previousValue >= minWindow - kComparisonEpsilon &&
	                       previousValue <= maxWindow + kComparisonEpsilon;
	if (wasInside) {
		return false;
	}
...
bool computeNextMoonPhaseOccurrenceForDate(
    const ESPDate &date, const Schedule &schedule, const DateTime &fromUtc, DateTime &outNextUtc
) {
	const DateTime rounded = roundToNextMinute(date, fromUtc);
...
bool computeNextMoonIlluminationOccurrenceForDate(
    const ESPDate &date, const Schedule &schedule, const DateTime &fromUtc, DateTime &outNextUtc
) {
	const DateTime rounded = roundToNextMinute(date, fromUtc);
	DateTime previous = date.addMinutes(rounded, -1);
...
bool computeNextOccurrenceForDate(
    const ESPDate &date, const Schedule &schedule, const DateTime &fromUtc, DateTime &outNextUtc
) {
...
ESPScheduler::ESPScheduler(ESPDate &date, ESPWorker *worker, const ESPSchedulerConfig &config)
    : m_date(date), m_minValidEpochSecondsRef(
                        std::make_shared<std::atomic<int64_t>>(kDefaultMinValidEpochSeconds)
                    ),
...
bool ESPScheduler::validateSchedule(const Schedule &schedule) const {
	...
	case ScheduleKind::Sunrise:
	case ScheduleKind::Sunset:
		return schedule.sunOffsetMinutes >= kMinSunOffsetMinutes &&
		       schedule.sunOffsetMinutes <= kMaxSunOffsetMinutes;
	case ScheduleKind::MoonPhaseAngle:
...
uint32_t ESPScheduler::addJobOnceUtc(
...
uint32_t ESPScheduler::addJob(
	...
	auto ctx = std::allocate_shared<WorkerJobContext>(
	    SchedulerAllocator<WorkerJobContext>(usePSRAMBuffers_)
	);
...
bool ESPScheduler::getJobInfo(size_t index, JobInfo &out) const {
	...
	for (const auto &job : m_workerJobs) {
		...
		if (current == index) {
			...
			fillNext(
			    job.context->schedule,
			    job.context->hasNext,
			    job.context->nextRunUtc,
			    out.nextRunUtc
			);
			return true;
		}
...
bool ESPScheduler::computeNextOccurrence(
    const Schedule &schedule, const DateTime &fromUtc, DateTime &outNextUtc
) const {
...
void ESPScheduler::runWorkerJob(const std::shared_ptr<WorkerJobContext> &ctx) {
	...
	while (!ctx->cancelRequested.load()) {
		...
		const int64_t minValidEpochSeconds = ctx->minValidEpochSeconds
		                                         ? ctx->minValidEpochSeconds->load()
		                                         : kDefaultMinValidEpochSeconds;
		...
		if (!ctx->hasNext) {
			if (isOneShotSchedule(ctx->schedule)) {
				...
				ctx->hasNext =
				    computeNextOccurrenceForDate(date, ctx->schedule, now, ctx->nextRunUtc);
				if (!ctx->hasNext) {
					break;
				}
			}
		...
		if (diffSec > 0) {
			const int64_t chunk =
			    (diffSec > kWorkerSleepChunkSeconds) ? kWorkerSleepChunkSeconds : diffSec;
